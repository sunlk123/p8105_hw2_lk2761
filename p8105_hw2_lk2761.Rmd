...
The Mr. Trash Wheel dataset consists of `r nrow(trash_wheel_data)` observations. The mean weight of trash removed by Mr. Trash Wheel in the Inner Harbor of Baltimore, Maryland between May 2014 and June 2019 is `r round(mean(pull(trash_wheel_data, weight_tons)), 2)` tons and the mean volume is `r round(mean(pull(trash_wheel_data, volume_cubic_yards)), 2)` cubic yards. The median number of sports balls removed in 2017 was `r median(pull(trash_wheel_data, sports_balls_int))` balls. Data on precipitation in 2017 and 2018 were combined into one dataset, which is made up of `r nrow(precip_combined)` observations. The average amount of rainfall from January 2017 to December 2018 was `r round(mean(pull(precip_combined, total_in)), 2)` inches. The total preciptation in 2018 was `r 
...
As a result of merging three separate datasets, the resulting dataset is `r nrow(pol_snp_unemploy_data)` rows by `r ncol(pol_snp_unemploy_data)` columns. The dataset includes data on national politicians (governors, senators, presidents and representatives) and whether they are Democratic or Republican at a given year, the closing values of the Standard & Poor's (S&P) stock market index in a given month and year, and the percentage of unemployment in a given month and year. 
...
Creating a reader-friendly table showing the rank in popularity of the name "Olivia" as a female baby name over time 
...
We are creating a reader-friendly table showing the most popular male name for all ethnicities and all years. 

```{r create table for males}
...
We will not create a scatterplot to display the data on the popularity of first names for male, white non-hispanic children born in 2016
